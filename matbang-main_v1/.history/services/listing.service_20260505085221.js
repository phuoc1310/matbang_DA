...
export async function getListings(filters) {
  ...
  // 💰 min price (triệu → đồng)
    ...
    // 🔎 type filter - also check title/description if `type` column is empty
    if (filters.type) {
      query += ` AND (LOWER(COALESCE(type, '')) LIKE LOWER($${i}) OR LOWER(COALESCE(title, '')) LIKE LOWER($${i}) OR LOWER(COALESCE(description, '')) LIKE LOWER($${i}))`;
