...
function updateWidget() {
  ...
  document.querySelectorAll('[class*="compare-btn-"]').forEach(btn => {
    // extract id from class compare-btn-{id}
...
function initAll() {
  if (!isInitialized) {
    isInitialized = true;
    initWidgetDOM();
    setTimeout(loadCompareList, 500); // Wait for auth to init
  }
}
...
// For monkey-patching renderPage
